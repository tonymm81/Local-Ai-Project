...
curl -sS -X POST "http://127.0.0.1:7860/api/generate"   -H "Content-Type: application/json"   -d '{"prompt":"What you like to do when you wake up in the morning","model":"pixtral-12b-q2:latest","conversation_title":"Aamurutiini?","user_id":"tester"

curl -sS -X POST "http://127.0.0.1:7860/api/generate" \
  -H "Content-Type: application/json" \
...
curl -v -X POST http://localhost:11440/generate \
  -H "Content-Type: application/json" \
  -d '{"prompt":"Hello QWEN, say hi","conversation_title":"MyTopic A","model":"qwen2.5:7b"}'
