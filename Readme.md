...
### version 113
- Adding the shutdown and update feature. I add this, because it would be nice to shutdown the linux server from app instead of connection with putty to server. 

#### Summary & Factuality Feature Testing Notes
During development of the summarize agent answer feature, we tested how different LLM‑generated responses behave when passed to a secondary model for summarization and factual accuracy evaluation.

##### Key findings:

The summarization model (Qwen) works reliably when the input text is natural, human‑like content.

However, responses generated by other models (e.g., Pixtral) often contain hidden structural patterns such as step‑lists, instructional formatting, or embedded code blocks.

These structural elements cause the summarization model to misinterpret the text as instructions rather than content, leading it to repeat the original text instead of summarizing it.

After cleaning the agent’s output (removing code blocks, numbered steps, and meta‑instruction patterns), the summarization model produced correct summaries and factuality scores.

This confirms that a preprocessing step is required before sending agent responses to the summarization/factuality module.

##### Conclusion:  
Implementing a text‑cleaning layer is necessary to ensure consistent behavior when summarizing and validating responses generated by different LLM agents.
